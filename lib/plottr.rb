...
module Plottr
  class Scatter
    def initialize(coords_array)
      gen_blank_scatter_plot(coords_array)
      add_coords_to_blank_plot(coords_array)
    end

    def gen_scatter_plot_as_string
      " ^\n" + @current_scatter_plot.reverse.join("\n") + "\n" +  @bottom_of_scatter_plot.join("\n")
    end

    private

    def add_coords_to_blank_plot(coords_array)
    ...
    def gen_blank_scatter_plot(coords_array)
      highest_x_val = find_highest_x(coords_array)
      highest_y_val = find_highest_y(coords_array)
      @bottom_of_scatter_plot = gen_bottom_of_scatter_plot(highest_x_val)
    ...
    def gen_bottom_of_scatter_plot(highest_x_val)
